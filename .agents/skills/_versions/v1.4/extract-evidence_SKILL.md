...
<!-- ruleset version v1.4 — log: methodology/RULESET_VERSIONS.md · snapshots: .agents/skills/_versions/.
...
15. **Species-specific evidence → TAG + KEEP, never drop → `species_envelope`** (2026-07). A
    ...
    **`claim_scope=species_specific`** (or `crop_specific`) **+ a `taxon`** (e.g. `Faidherbia
    ...
    RETAINS it in the register for a future species-level layer. The defect is the **mis-tag**:
