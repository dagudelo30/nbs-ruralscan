...
def load_scorecard(t6: pd.DataFrame, t5_all: pd.DataFrame, t2: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    valid_ids = set(t5_all["variable"]) | set(t2["hazard_type"].dropna()) | {
    ...
    cols = ["variable_id", "effect_direction", "effect_confidence", "effect_mechanism", "conditionality"]
    return t6[cols]


def pair_with_problems(scorecard: pd.DataFrame, problem_distribution: pd.DataFrame) -> pd.DataFrame:
...
def run_m5(t6: pd.DataFrame, t5_all: pd.DataFrame, t2: pd.DataFrame, problem_distribution: pd.DataFrame) -> dict:
